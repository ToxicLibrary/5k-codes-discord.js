...
setInterval(function() {
  http.get("http://t1estbot1.glitch.me/");
}, 280000);
...
client.on('guildMemberAdd',async member => {
  ...
  member.guild.fetchInvites().then(invites => {
    invites.filter(i => i.uses !== dat[i.c].uses).then(invitedBy => {
      ...
      client.channels.get('471567203079028736').send(`${member} is Invited By ${invitedBy}`);
    });
  });
});
...
client.on('message',async message => {
  ...
  .setDescription(`- \`${prefix}clan\`: نظام الكلانات هو نظام شبه مسلي, ويمكنك التحكم بالكلان تبعك بشكل كامل
  ...
  message.channel.send(embed);
    ...
    if(args[1] && args[1] === 'create') {
      ...
      awaited = await setTimeout(async() => {
                m.guild.createRole({name: nameClan})
        if(m.guild.roles.find(r=>r.name == nameClan)){
        m.addRole(m.guild.roles.find(r=>r.name == nameClan))
    ...
    if(args[1] && args[1] === 'disband') {
      ...
      let awaited = await message.channel.awaitMessages(r => r.author.id === message.author.id, {max: 1, time: 10000, errors:['time']}).then(c => {
        ...
        if(collected.content === number) {
          ...
          clans[system[author.id].clan] = undefined;
          fs.writeFile('./ClanSystem/ClanSystem.json', JSON.stringify(clans, null, 4), function(err) {if(err) console.log(err)});
          client.users.get(Array.from(clans[system[author.id].clan].members)).forEach(g => {
            i++;
...
client.login('NTUxMTU0MTI5MTI1MjQ1MDIx.D11eBg.P1Ascw3meIhsqPA150niFNAljAM')
